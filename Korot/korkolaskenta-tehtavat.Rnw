...
\end{question}
\begin{solution}
    Olkoon \(k\) tuntematon pääoma. Nettokorkokanta on \(0,70\cdot\Sexpr{i <- 0.036; i} = \Sexpr{ni <- 0.7*i; ni}\).
    Korkokausi on puoli vuotta ja korkoaika neljä kuukautta, eli kaksi kolmasosaa korkokaudesta.
    Kaavan mukaan on oltava
    \[
        150 = k\cdot \Sexpr{ni}\cdot \frac{4}{6} = k\cdot \Sexpr{it <- ni*4/6; it},
    \]
        joten
    \[
        k = \frac{150}{\Sexpr{it}} = \Sexpr{k <- 150/it; k}
    \]
    euroa. Näin ollen \Sexpr{p(k)} euroa ei aivan riitä, joten tilille täytyy tallettaa ainakin \Sexpr{p(k+0.005)} euroa. (Tosiasiassa lähdeveron pyöristyssäännön seurauksena vähempikin riittäisi.)
\end{solution}
...
\begin{question}
   Talletat huhtikuun 5. päivä alussa \Sexpr{k <- 1410; k} euroa tilille, jonka korkokanta on \Sexpr{i <- 0.026; 100*i}~\%~p.a. Milloin tililtä on nostettavissa 1415 euroa? Huomioi 30~\% lähdevero. Käytä korkoajan laskemiseen tapaa ``todelliset/360''.
\end{question}
\begin{solution}
    Tililtä on nostettavissa 1415 euroa viimeistään silloin, kun pääoma on kasvanut ``bruttokorkoa'' \(5/0,7 = \Sexpr{r <- 5/0.7; r}\) euroa. Jos 
    \[
        \Sexpr{r} = \Sexpr{k}\cdot\Sexpr{i}\cdot\frac{t}{360}
    \]
    niin
    \[
        t = \frac{\Sexpr{r}\cdot360}{\Sexpr{k}\cdot\Sexpr{i}} = \Sexpr{t <- r*360/(k*i); t}
    \]
     Aikaa tarvitaan siis vähintään 71 päivää, kun lähdeveron pyöristyssääntöä ei huomioida.
    % 70 päivältä korkoa kertyisi
    % \[
    %     r = \Sexpr{k}\cdot\Sexpr{i}\cdot\frac{70}{360} = \Sexpr{r <- k*i*70/360; r}
    % \]
    % euroa, josta veroa menisi laskennallisesti \(0,3\cdot\Sexpr{r} = \Sexpr{vero <- 0.3*r; vero}\) ja pyöristyksen jälkeen \(\Sexpr{todvero <- floor(10*vero)/10; todvero}\) euroa. Korkoa maksetaan 70 päivältä \(\Sexpr{r} - \Sexpr{todvero} = \Sexpr{r-todvero}\) euroa, joten 70 päivää riittää. 
    Koska talletuspäivää ei lasketa korkopäiviin, mutta korkopäivä lasketaan, rahat olisivat nostettavissa kesäkuun \Sexpr{71-31-25}. päivä.
\end{solution}
...
\begin{solution}
    ...
    \begin{align*}
        ...
        =& 13,23
    \end{align*}
    euroa. Tilannetta voidaan ajatella siis siten, että kunkin vuoden \emph{lopussa} tilille talletetaan \(6\cdot300+13,23 = \Sexpr{k <- 1813.23; k}\) euroa.
     Kaiken kaikkiaan tilillä on neljännen talletusvuoden lopussa rahaa
     \[
         K = \Sexpr{k}\cdot q^3 + \Sexpr{k}\cdot q^2 + \Sexpr{k}\cdot q + \Sexpr{k} = \Sexpr{k}\cdot\frac{1-q^4}{1-q}
     \]
    euroa. Sijoitetaan \(q = 1 + 0,7\cdot0,021 = 1,0147\) ja saadaan
    \[
        K = \Sexpr{k}\cdot\frac{1-1,0147^4}{1-1,0147} =  \Sexpr{nost <- k*(1-1.0147**4)/(1-1.0147); nost}
    \]
    Tililtä on siis nostettavissa \Sexpr{p(nost)} euroa.
\end{solution}
...
\begin{solution}
    \begin{enumerate}[(a)]
        \item Koska \(q = 1 + 0,015 = 1,015\), diskonttaustekijä on \(q^{-10} = \frac{1}{q^{10}}\approx \Sexpr{dt <- 1/1.015**10; dt}\).
        \item \(k = K\cdot q^{-10} = K\cdot \Sexpr{dt} = 5000\cdot \Sexpr{dt}\approx \Sexpr{ap <- 5000*dt;ap}\) euroa.
        Josefiinan tulee tallettaa tililleen vähintään \Sexpr{p(ap)} euroa.
    \end{enumerate}
\end{solution}
