...
\begin{frame}
    \frametitle{Esimerkki}
      Tilille, jonka korkokanta on \Sexpr{i <- 0.022; i*100}~\%~p.a. talletetaan \Sexpr{k <- 2650; k} euroa. Puolessa vuodessa \Sexpr{t <- 0.5} talletus on kasvanut korkoa
      \[
        r = kit = \Sexpr{k}\cdot\Sexpr{i}\cdot\Sexpr{t} = \Sexpr{r <- k*i*t; r}
      \] euroa. Ennen maksamista korosta peritään 30~\% lähdeveroa, joten maksettavaksi koroksi, eli \emph{nettokoroksi} jää 
      \[
        0,70\cdot\Sexpr{r} \approx \Sexpr{p(0.7*r)}
      \] euroa. Samaan tulokseen päästään, jos korkokannan \(i\) tilalla käytetään \emph{nettokorkokantaa} \(0,7\cdot\Sexpr{i} = \Sexpr{ni <- 0.7*i; ni}\): tällöin
      \[
        r = \Sexpr{k}\cdot\Sexpr{ni}\cdot\Sexpr{t} \approx \Sexpr{p(k*ni*t)}
      \]
      euroa.
...
\begin{frame}
    ...
    \pause
    \begin{block}{}
        Edellisessä esimerkissä lähdevero \(0,30\cdot\Sexpr{r} = \Sexpr{nr <- 0.3*r; nr}\)~\euro\ pyöristettäisiin ja todellisuudessa perittävä vero olisi \Sexpr{rnr <- floor(10*nr)/10; rnr} euroa; tällöin nettokoroksi jäisi \Sexpr{r-rnr} euroa (kun se edellä oli \Sexpr{p(0.7*r)} euroa).
    \end{block}
    \pause
...
\begin{frame}
    ...
    \begin{scriptsize}
        <<echo=F,results='asis'>>=
        library('xtable')
        kuut <- c("Maalis", "Huhti", "Touko", "Kesä", "Heinä", "Elo", "Syys", "Loka", "Marras", "Joulu")
        df <- data.frame(kuut, 9:0, row.names=1)
        print(xtable(t(df)), include.rownames=F)
        @
    \end{scriptsize}
    ...
    \pause Saadaan
    \[
        r = 100\cdot0,015\cdot\frac{\Sexpr{t <- sum(1:9); t}}{12} = \Sexpr{r <-100*0.015*t/12; r},
    \]
    eli korkoa maksetaan (lähdeveron jälkeen) \(0,7\cdot\Sexpr{r} \approx \Sexpr{p(0.7*r+0.0005)}\) euroa.
\end{frame}

\begin{frame}
    \frametitle{Koronkorkolaskentaa (esimerkki)}
    \pause
    Säästötilille, jonka korkokanta on 1,5~\% p.a. talletetaan \Sexpr{k <- 1500; k} euroa. \pause Vuoden kuluttua talletus kasvaa korkoa \(0,7\cdot 1,5 = \Sexpr{i <- 0.7*0.015; i*100}\) prosenttia\pause, eli tilillä on 
    \[
        \Sexpr{q <- 1+i; q}\cdot\Sexpr{k} = \Sexpr{k1 <- q*k; k1}
    \]
    euroa. \pause Seuraavan vuoden aikana tämä summa kasvaa korkoa \Sexpr{i*100} prosenttia, joten tilillä on toisen vuoden lopussa
    \[
        \Sexpr{q}\cdot\Sexpr{k1} \approx \Sexpr{p(q*k1)}
    \]
    euroa jne. \pause Esimerkiksi 10 vuoden kuluttua tilillä olisi siis
    \[
        \Sexpr{k}\cdot\underbrace{\Sexpr{q}\cdots\Sexpr{q}}_{10\text{ kpl}} = \Sexpr{k}\cdot\Sexpr{q}^{10} \approx \Sexpr{p(k*q**10)}
    \]
    euroa.
\end{frame}
...
\begin{frame}
    \frametitle{Esimerkki}  
    Säästötilille, jonka korkokanta on \Sexpr{i <- 0.027; 100*i}~\% p.a. talletetaan \Sexpr{k <- 900; k} euroa vuosittain aina vuoden alussa. 
    \pause 
    Tilin nettokorkokanta on \(0,7\cdot\Sexpr{i} = \Sexpr{ni <- 0.7*i; ni}\), joten kunkin vuoden talletus tulee \Sexpr{q <- 1+ni; q}-kertaiseksi vuoden välein. 
    \pblock 
    Viiden vuoden kuluttua ensimmäinen talletus on tullut \(\Sexpr{q}^5\)-kertaiseksi\pause, toisen vuoden talletus \(\Sexpr{q}^4\)-kertaiseksi\pause, kolmannen vuoden \(\Sexpr{q}^3\)-kertaiseksi jne. \pause Kaiken kaikkiaan tilillä on silloin rahaa (ennen viidennen vuoden talletusta)
    \begin{multline*}
        \Sexpr{q}^5\cdot\Sexpr{k} + \Sexpr{q}^4\cdot\Sexpr{k} + \Sexpr{q}^3\cdot\Sexpr{k} \\  + \Sexpr{q}^2\cdot\Sexpr{k} + \Sexpr{q}^1\cdot\Sexpr{k} \approx \Sexpr{K <- sum(q**(1:5))*k; p(K)}
    \end{multline*}
    euroa. 
    \pause 
    Korkoa on siis saatu \Sexpr{p(K-5*k)} euroa.
\end{frame}
