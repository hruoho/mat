...
%     \pause
%     \begin{ratkaisu}
%         Kun lähdevero otetaan huomioon, korkokanta on \(0,70\cdot 0,022 = \Sexpr{i <- 0.7*0.022; i}\). \pause Veron suuruus on silloin
%         \[
%             r = kit = 2500\cdot \Sexpr{i} \cdot \frac{8}{12}\approx 25,67
%          \]
%          euroa.
%     \end{ratkaisu}
% \end{frame}
...
%       \[
%           r = kit = 700\cdot 0,70\cdot0,0075\cdot\frac{237}{360} \approx \Sexpr{p(237/360)}
%       \]
%       tai
%       \[
%           r = kit = 700\cdot 0,70\cdot0,0075\cdot\frac{237}{365} \approx \Sexpr{p(237/365)}
%       \]
%       euroa.
%     \end{ratkaisu}
% \end{frame}
...
% %= 100\cdot0,015\cdot\frac{\Sexpr{t <- sum(1:9); t}}{12} = \Sexpr{100*0.015*t/12}
% 	\begin{ratkaisu}		
% 		\pause Tämä saadaan muotoon
%     \[
%     100\cdot0,015\cdot\frac{1}{12}\cdot(9+8+\cdots+1)
%     \]
%     eli koron suuruus on \pause
%     \[
%     \frac{1,5}{12}\cdot(9+8+\cdots+1)
%     \]
%     euroa. 
%   Esimerkiksi aritmeettisen summan kaavaa käyttämällä koroksi saadaan
%     \[
%     r = \frac{1,5}{12}\cdot\left( 9\cdot\frac{9+1}{2}\right) = 5,625
%     \]
%     euroa. \pause Lähdeveron jälkeen nettokoroksi jää \(0,70\cdot 5,625=3,94\) euroa, kun pyöristyssääntö jätetään huomiotta.
% 	\end{ratkaisu}
% \end{frame}

\begin{frame}
\frametitle{Koronkorkolaskentaa (esimerkki)}
		Säästötilille, jonka korkokanta on 1,5~\% p.a. talletetaan \Sexpr{k <- 1500; k} euroa. Vuoden kuluttua talletus kasvaa korkoa \(0,7\cdot 1,5 = \Sexpr{i <- 0.7*0.015; i*100}\) prosenttia, eli tilillä on 
    \[
    \Sexpr{q <- 1+i; q}\cdot\Sexpr{k} = \Sexpr{k1 <- q*k; k1}
    \]euroa. Seuraavan vuoden aikana tämä summa kasvaa korkoa \Sexpr{i*100} prosenttia, joten tilillä on 
    \[
